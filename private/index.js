...
app.get('/*', function (req, res) {
	...
	if(req.originalUrl === '' || req.originalUrl === '/'){
		...
		file = req.originalUrl.replace('../', '');
	}
	if(fs.existsSync(__dirname + "/../public" + file)){
		res.send(fs.readFileSync(__dirname + "/../public" + file));
		res.status(200).end();
	} else {
		res.send(fs.readFileSync(__dirname + "/../public/404.html"));
		res.status(404).end();
	}
});
...
function checkForWin(game, data){
	...
	if(winner !== ''){
		...
		game.players[data.type].opponent.socket.emit("loser");
